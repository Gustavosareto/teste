...
const billStatusClasses = {
  ...
  pago: 'bg-emerald-100 dark:bg-emerald-900/30 text-emerald-800 dark:text-emerald-300 border-emerald-200 dark:border-emerald-800',
};
    ...
    <section className="space-y-8" aria-labelledby="bills-heading">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        ...
        <aside className="lg:col-span-5 xl:col-span-4 lg:sticky lg:top-8">
          ...
          <div className="mt-6 rounded-2xl bg-amber-50 dark:bg-amber-950/30 border border-amber-100 dark:border-amber-900/50 p-5 shadow-md dark:shadow-black/20">
            <Typography variant="label" className="text-amber-800 dark:text-amber-500">
              Total pendente no mês
            </Typography>
            {isPending ? (
              <Skeleton width="w-32" height="h-8" className="mt-2" rounded="md" />
            ) : (
              <Typography variant="h2" as="p" className="mt-2 text-amber-900 dark:text-amber-400">
        ...
        <div className="lg:col-span-7 xl:col-span-8 space-y-4" aria-live="polite">
          ...
          {!isPending && !isError && displayedBills.length === 0 && (
            <div className="py-16 text-center border-2 border-dashed border-slate-200 dark:border-slate-800 rounded-2xl bg-slate-50 dark:bg-slate-800/50/50 flex flex-col items-center gap-4">
          ...
          {!isPending && !isError && displayedBills.map((bill) => (
            <article 
              key={bill.id} 
              className="group flex flex-col sm:flex-row sm:items-center justify-between gap-4 rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-5 shadow-md dark:shadow-black/20 hover:shadow-md hover:border-slate-300 dark:border-slate-700 transition-all cursor-default relative"
            >
              ...
              <div className="flex items-center gap-4 pr-16">
                <div className="hidden sm:flex h-12 w-12 items-center justify-center rounded-lg bg-slate-50 dark:bg-slate-800/50 border border-slate-100 dark:border-slate-700 text-slate-400 group-hover:text-slate-900 dark:text-slate-100 transition-colors">
                ...
                <div>
                  <Typography variant="h4" as="h2" className="text-slate-900 dark:text-slate-100">
                    {bill.name}
                  </Typography>
              ...
              <div className="flex items-center justify-between sm:flex-col sm:items-end gap-2 border-t border-slate-100 dark:border-slate-700 sm:border-0 pt-4 sm:pt-0 mt-4 sm:mt-0">
                <Typography variant="h3" as="p" className="text-slate-900 dark:text-slate-100">
                  <CurrencyFormat value={bill.amount} type="expense" />
                </Typography>
      ...
      <ConfirmModal
        ...
        message="Tem certeza que deseja excluir esta despesa? As movimentações atreladas a ela (caso já tenham sido pagas) não serão alteradas, mas a despesa não aparecerá mais aqui."
        confirmText="Excluir"
        isDestructive={true}
        onConfirm={() => {
          if (billToDelete) {
